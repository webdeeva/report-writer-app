 ...
 */
const TextDateInput = ({ value, onChange, id, placeholder = 'MM/DD/YYYY', disabled = false, required = false, className = '', 'aria-invalid': ariaInvalid, 'aria-describedby': ariaDescribedby }) => {
    ...
    return (_jsxs("div", { className: `relative ${className}`, children: [_jsx("input", { id: inputId, type: "text", value: displayValue, onChange: handleChange, onBlur: handleBlur, placeholder: placeholder, disabled: disabled, required: required, className: `
        ...
        `, "aria-invalid": ariaInvalid || (error ? 'true' : 'false'), "aria-describedby": ariaDescribedby }), error && (_jsx("p", { className: "mt-1 text-sm text-red-600", role: "alert", children: error }))] }));
};
